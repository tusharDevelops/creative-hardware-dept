...
export function NewSaleClient() {
  ...
  const [step, setStep] = useState(1) // 1: Select Customer, 2: Add Items/Review
  ...
  // Fetch quotation if provided
  useEffect(() => {
    if (quotationId) {
      const fetchQuotation = async () => {
        try {
          const res = await fetch(`/api/quotations?search=${quotationId}`)
          const json = await res.json()
          const quote = json.data?.find((q: any) => q.id === quotationId)
          if (quote) {
            ...
            setTotals({
              ...
              gst: quote.gst,
              transport: quote.transportCharge,
              grandTotal: quote.grandTotal
            })
            // remarks doesn't exist as state here if it wasn't added yet, need to check
            setStep(2)
          }
        } catch (e) {
              ...
              <Card className="w-full max-w-md animate-in slide-in-from-bottom-10 bg-canvas rounded-[var(--radius-xl)]">
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    <Button className="flex-1" onClick={handleAddItem}>Add to Sale</Button>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="fixed bottom-16 sm:bottom-0 left-0 w-full bg-canvas border-t border-hairline p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-40">
              <div className="max-w-2xl mx-auto space-y-3">
                ...
                <div className="flex justify-between items-end border-t border-hairline pt-3">
                  ...
                  <Button className="px-8" onClick={handleSaveSale}>
                    Generate PDF
                  </Button>
                </div>
              </div>
